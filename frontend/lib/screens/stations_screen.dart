...
class _StationsScreenState extends State<StationsScreen>
  ...
  Future<void> _loadStations() async {
    ...
    try {
      ...
      final mapped = raw.map<Map<String, dynamic>>((station) {
        return {
          'id': station['id'],
          'stopId': station['id'],
          'name': station['name'],
          ...
          'safetyLevel': (station['safety_level'] ?? '').toString().toLowerCase(),
          'recentReportsCount': station['recent_reports'] ?? 0,
          'averageRating': 3.5,
          'operatingHours': ['05:00 - 22:00'],
          'distance': station.containsKey('distance_km') ? station['distance_km'] : 0.0,
          'upvotes': 0,
          'downvotes': 0,
  ...
  List<Map<String, dynamic>> _getFilteredStations() {
    return _stations.where((station) {
      ...
      bool matchesType = _selectedTransportType == 'all' ||
          station['type'] == _selectedTransportType;

      bool matchesSafety = _selectedSafetyLevel == 'all' ||
  ...
  void _planRoute(Map<String, dynamic> station) {
    Navigator.pop(context);
    Navigator.pushNamed(context, '/route-planning', arguments: {'destination': station['name']});
  }
                ...
                child: ListView(
                  ...
                  children: [
                    ...
                    _buildDetailSection('Location', [
                      _buildDetailRow('Address', station['location']['address'], Icons.location_on),
                      _buildDetailRow('Distance', '${station['distance']} km away', Icons.directions),
                      _buildDetailRow(
                          'Coordinates',
                    ...
                    _buildDetailSection('Safety Information', [
                      _buildDetailRow('Safety Level', _getSafetyText(station['safetyLevel']), _getSafetyIcon(station['safetyLevel'])),
                      _buildDetailRow('Recent Reports', '${station['recentReportsCount']} in last 7 days', Icons.report),
                      _buildDetailRow('Community Rating', '${station['averageRating']}/5.0 stars', Icons.star),
                      _buildDetailRow('Community Votes', '${station['upvotes']} 👍 ${station['downvotes']} 👎', Icons.thumb_up),
                    ]),
                    SizedBox(height: 16),
                    _buildDetailSection('Service Information', [
                      _buildDetailRow('Transport Type', station['type'].toUpperCase(), _getTransportIcon(station['type'])),
                      _buildDetailRow('Operating Hours', station['operatingHours'][0], Icons.access_time),
                      _buildDetailRow('Last Updated', _getTimeAgo(station['lastUpdated']), Icons.update),
                    ]),
                    SizedBox(height: 24),
                    Row(
                      children: [
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Expanded(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(label, style: TextStyle(fontWeight: FontWeight.w600, fontSize: 14, color: Colors.grey[600])),
            SizedBox(height: 2),
  ...
  Widget _buildSearchAndSummary() {
    ...
    final safeStations = _getFilteredStations().where((s) => s['safetyLevel'] == 'green').length;
    final unsafeStations = _getFilteredStations().where((s) => s['safetyLevel'] == 'red').length;
    ...
    return Container(
      ...
      child: Column(children: [
        ...
        SizedBox(height: 16),
        Row(children: [
          Expanded(child: _buildSummaryCard('Total', totalStations.toString(), Icons.train, AppColors.primaryBlue)),
          SizedBox(width: 12),
          Expanded(child: _buildSummaryCard('Safe', safeStations.toString(), Icons.check_circle, AppColors.primarySafetyGreen)),
          SizedBox(width: 12),
          Expanded(child: _buildSummaryCard('Unsafe', unsafeStations.toString(), Icons.warning, AppColors.alertRed)),
        ]),
      ]),
    );
  }
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              Container(
                ...
                child: Icon(_getTransportIcon(station['type']), color: AppColors.primaryBlue, size: 24),
              ),
              SizedBox(width: 12),
              Expanded(
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text(station['name'], style: Theme.of(context).textTheme.headlineMedium?.copyWith(fontSize: 16)),
                  SizedBox(height: 2),
                  Text(station['location']['address'], style: TextStyle(color: Colors.grey[600], fontSize: 12)),
                ]),
              ),
              Container(
                padding: EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                decoration: BoxDecoration(color: safetyColor.withOpacity(0.1), borderRadius: BorderRadius.circular(12)),
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  Icon(_getSafetyIcon(station['safetyLevel']), size: 14, color: safetyColor),
                  SizedBox(width: 4),
                  Text(safetyText.toUpperCase(), style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: safetyColor)),
                ]),
              ),
            ]),
            SizedBox(height: 12),
            Row(children: [
              _buildStatChip(Icons.star, (station['averageRating'] as double).toStringAsFixed(1), AppColors.warningAmber),
              SizedBox(width: 8),
              _buildStatChip(Icons.location_on, '${station['distance']} km', AppColors.primaryBlue),
              SizedBox(width: 8),
              if ((station['recentReportsCount'] as int) > 0)
                _buildStatChip(Icons.report, '${station['recentReportsCount']} reports', AppColors.alertRed),
              Spacer(),
              Text('Updated ${_getTimeAgo(station['lastUpdated'])}', style: TextStyle(fontSize: 10, color: Colors.grey[500])),
            ]),
            SizedBox(height: 8),
            Row(children: [
              Icon(Icons.access_time, size: 14, color: Colors.grey[600]),
              SizedBox(width: 4),
              Text('Hours: ${station['operatingHours'][0]}', style: TextStyle(fontSize: 12, color: Colors.grey[600])),
            ]),
          ]),
        ),
      ),
  ...
  Widget _buildEmptyState() {
    return Center(
      child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        ...
        SizedBox(height: 16),
        Text('No stations found', style: Theme.of(context).textTheme.headlineMedium?.copyWith(color: Colors.grey)),
        SizedBox(height: 8),
        Text('Try adjusting your filters or search terms', style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: Colors.grey)),
      ]),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(icon: Icon(Icons.filter_alt), onPressed: _showFilterDialog),
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  SizedBox(width: 8),
                  Expanded(child: Text('Failed to load stations: $_error')),
